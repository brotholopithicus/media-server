...
fetch('/api')
  ...
  .then(res => {
    res.forEach(file => {
      ...
      download.addEventListener('click', () => downloadPlayList(`http://${file.ipv4Addr}:3000/api/link/${file.id}`));
      copyLink.addEventListener('click', () => copyTextToClipboard(`http://${file.ipv4Addr}:3000/api/video/${file.id}`));
